#ifndef GL_COMPATIBILITY_PATCH_H
#define GL_COMPATIBILITY_PATCH_H

/*
	gl_compatibility_patch
	
	Copyright (c) 2025 Paul Jankowski
	
	with exceptions of parts copied from:
		1. mingw-w64 gl.h - which is in public domain
		2. Mesa 3-D gl.h which is licensed under MIT License:
			Copyright (C) 1999-2006  Brian Paul   All Rights Reserved.
			Copyright (C) 2009  VMware, Inc.  All Rights Reserved.
	
	
		in this file there is everything that is
		present in gl.h,
		but not in glcorearb.h and glext.h
		
		include this instead gl.h,
		AFTER glcorearb.h and glext.h
		
		note: for function prototypes used:
		#ifdef GL_GLEXT_PROTOTYPES
		
		analysis indicated that its compatible with
		gl.h from WinAPI 10.0.26100.0
		
		+ bonus:
		typedefs of some GLX functions
...
/*
	this typedefs are done manually for project purpose
	this list is incomplete
	gl convention is followed
	
	static linking not tested
*/
#if defined _WIN32
	#define GL_COMPAT_API __cdecl
#else
	#define GL_COMPAT_API
#endif

typedef void (GL_COMPAT_API * PFNGLALPHAFUNC)(GLenum func, GLfloat ref);
typedef void (GL_COMPAT_API * PFNGLBEGINPROC)(GLenum mode);
typedef void (GL_COMPAT_API * PFNGLCALLLISTPROC)(GLuint list);
typedef void (GL_COMPAT_API * PFNGLCOLOR4FPROC)(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha);
typedef void (GL_COMPAT_API * PFNGLDELETELISTSPROC)(GLuint list, GLsizei range);
typedef void (GL_COMPAT_API * PFNGLEDGEFLAGPROC)(GLboolean flag);
typedef void (GL_COMPAT_API * PFNGLENDPROC)(void);
typedef void (GL_COMPAT_API * PFNGLENDLISTPROC)(void);
typedef GLuint (GL_COMPAT_API * PFNGLGENLISTSPROC)(GLsizei range);
typedef void (GL_COMPAT_API * PFNGLLOADIDENTITYPROC)(void);
typedef void (GL_COMPAT_API * PFNGLLOADMATRIXF)(const GLfloat *m);
typedef void (GL_COMPAT_API * PFNGLMATRIXMODEPROC)(GLenum mode);
typedef void (GL_COMPAT_API * PFNGLNEWLIST)(GLuint list, GLenum mode);
typedef void (GL_COMPAT_API * PFNGLNORMAL3FPROC)(GLfloat nx, GLfloat ny, GLfloat nz);
typedef void (GL_COMPAT_API * PFNGLTEXCOORD4FPROC)(GLfloat s, GLfloat t, GLfloat r, GLfloat q);
typedef void (GL_COMPAT_API * PFNGLTEXENVF)(GLenum target, GLenum pname, GLfloat param);
typedef void (GL_COMPAT_API * PFNGLTEXENVFV)(GLenum target, GLenum pname,const GLfloat *params);
typedef void (GL_COMPAT_API * PFNGLVERTEX4FPROC)(GLfloat x, GLfloat y, GLfloat z, GLfloat w);
/*
	end of typedefs for project purpose
*/
...
// the following are from gl.h from libgl-dev ubuntu package
...
#ifdef GL_GLEXT_PROTOTYPES
void APIENTRY glBlendEquationSeparateATI(GLenum modeRGB,GLenum modeA);
#endif //GL_GLEXT_PROTOTYPES
...
#ifdef GL_GLEXT_PROTOTYPES
void APIENTRY glEGLImageTargetTexture2DOES (GLenum target,GLeglImageOES image);
void APIENTRY glEGLImageTargetRenderbufferStorageOES (GLenum target,GLeglImageOES image);
#endif //GL_GLEXT_PROTOTYPES
#endif

/*
		+ bonus: typedefs of some glx functions
*/
#if !(defined _WIN32)
...
#endif //GL_COMPATIBILITY_PATCH_H